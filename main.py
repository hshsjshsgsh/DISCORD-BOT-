...
tp_data = {}  # {guild_id: {user_id: tp_amount}}
...
def add_tp(guild_id, user_id, tp):
    guild_str = str(guild_id)
    user_str = str(user_id)
...
async def on_message(message):
    ...
    if guild_id and guild_id in leveling_settings and leveling_settings[guild_id]['enabled'] and leveling_settings[guild_id]['channel']:
...
@bot.command()
@commands.has_any_role("👑・Owner", "🛡️・Admin", "⚖️・Moderator", "🤝・Helper")
...
async def bracketrolereset(ctx, member: discord.Member = None):
    ...
    if str(member.id) in bracket_roles:
        del bracket_roles[str(member.id)]
        save_data()
...
async def svd(ctx):
    ...
    try:
        ...
        success_embed.set_footer(text="Your server has been completely recreated with proper hierarchy!")
...
async def commands(ctx):
    ...
    if ctx.author.guild_permissions.manage_channels:
        ...
        embed.add_field(
            name="🏷️ Bracket Management (Staff)",
...
async def winner(ctx, member: discord.Member):
    ...
    if len(tournament.results) == len(current_round):
        if len(tournament.results) == 1:
            ...
            # Create styled tournament winners embed similar to the image
            embed = discord.Embed(
                title="🏆 Tournament Winners!",
                description=f"Congratulations to **{get_player_display_name(placements[0][1][0]) if tournament.mode == '2v2' else get_player_display_name(placements[0][1])}** for winning the\n**{tournament.title}** tournament! 🎉",
                color=0xffd700
            )
            ...
            for place, player_or_team, tp in placements:
                ...
                if tournament.mode == "2v2":
                    team_str = f"{get_player_display_name(player_or_team[0])} & {get_player_display_name(player_or_team[1])}"
                else:
                    team_str = get_player_display_name(player_or_team)
            ...
            for place, player_or_team, tp in placements:
                if place == 1:
                    ...
                    emoji = "4️⃣"
                prize_text += f"{emoji} {place}{'st' if place == 1 else 'nd' if place == 2 else 'rd' if place == 3 else 'th'}: {tp} TP\n"
            
            embed.add_field(name="🏆 Prizes", value=prize_text, inline=False)
            
            # Add winner's avatar if it's a real player
            if tournament.mode == "2v2":
                winner_player = placements[0][1][0] if hasattr(placements[0][1][0], 'display_avatar') else placements[0][1][1]
            else:
                winner_player = placements[0][1]
                
            if hasattr(winner_player, 'display_avatar') and not isinstance(winner_player, FakePlayer):
                embed.set_thumbnail(url=winner_player.display_avatar.url)
