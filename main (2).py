...
# Load data
def load_data():
    global tp_data, role_permissions, bracket_roles
    try:
        with open('user_data.json', 'r') as f:
            data = json.load(f)
            # Support both old SP data and new TP data for migration
            tp_data = data.get('tp_data', data.get('sp_data', {}))
...
class TournamentView(discord.ui.View):
    ...
    async def register_button(self, interaction: discord.Interaction,
        ...
        try:
            ...
            await interaction.followup.send(
                f"✅ {interaction.user.display_name} registered! ({len(tournament.players)}/{tournament.max_players})",
                ephemeral=True)
        ...
        try:
            ...
            for i, match in enumerate(round_pairs, 1):
                ...
                embed.add_field(
                    name=f"⚔️ Match {i}",
                    value=
                    f"**{player_a}** <:VS:1402690899485655201> **{player_b}**\n<:Crown:1409926966236283012> Winner: *Waiting...*",
                    inline=False)
...
async def winner(ctx, member: discord.Member):
        ...
        try:
            ...
            if match_index >= 0 and match_index < len(current_embed.fields):
                ...
                if "Match" in field.name:
                    ...
                    lines[
                        1] = f"<:Crown:1409926966236283012> Winner: **{get_player_display_name(member, ctx.guild.id)}**"
    ...
    if len(tournament.results) == len(current_round):
        if len(tournament.results) == 1:
            ...
            placements = []  # List of (place, player, tp_reward)

            # 1st place (winner) - 100 TP
            ...
            # 2nd place (last eliminated) - 70 TP
            ...
            # Create results text
            results_display = ""
            for place, player_obj, tp in placements:
                if place == 1:
                    emoji = "🥇"
                elif place == 2:
                    emoji = "🥈"
            ...
            for place, player_obj, tp in placements:
                ...
                prize_text += f"{emoji} {place}{place_suffix}: {tp} Tournament Points\n"
...
async def tp_lb(ctx):
    ...
    # Sort players by TP
    sorted_players = sorted(guild_tp_data.items(),
                            key=lambda x: x[1],
                            reverse=True)[:10]

    embed = discord.Embed(title="🏆 Tournament Points Leaderboard",
                          color=0xf1c40f)
        ...
        for i, (user_id, tp) in enumerate(sorted_players, 1):
            ...
            if user:
                ...
                if i == 1:
                    ...
                    emoji = f"**{i}.**"
                leaderboard_text += f"{emoji} {user.display_name} - {tp} TP\n"
...
async def tp_rst(ctx):
    ...
    if guild_str in tp_data:
        ...
        save_data()
        await ctx.send(
            "✅ All Tournament Points have been reset for this server!",
            delete_after=5)
    else:
        await ctx.send("✅ No Tournament Points to reset in this server!",
                       delete_after=5)
